...
async function upsertPair() {
  const cfg = getPairConfig("EURJPY");
...
async function main() {
  ...
  console.log(`Backfill EURJPY 1h: ${startIso} -> ${endIso}`);

  const pair = await upsertPair();

  console.log("Fetching 1h bars from yfinance-service...");
  const bars1h = await fetchFxIntraday("EURJPY", "1h", startIso, endIso);
